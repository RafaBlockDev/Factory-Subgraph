import { Bytes, ipfs, json, JSONValue, JSONValueKind, TypedMap } from "@graphprotocol/graph-ts";
import { decode } from "as-base64";
import { parseUrl, CustomURL } from "../utils/urlParse";

const handleTokenURL = (url: CustomURL): TypedMap<string, JSONValue> | null => {
    if(url.protocol === "ipfs:") {
        const ipfsHASHI = url.pathname.slice(2);
        const bytes = ipfs.cat(ipfsHASHI);
        if(!bytes) {
            return null;
        }
...
export const parseTokenURI = (uri: string): TypedMap<string, JSONValue> | null => {
